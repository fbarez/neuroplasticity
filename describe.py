from identify_neurons import scrape_neuron_max_activations
from verify import (
    calculate_similarity,
    calculate_phrase_similarity,
    substitute_similar_phrases,
)
import openai
...
def ask_summary(keywords_list):
    ...
    prompt_summary = (
        "What do all these phrases have in common? Phrases: "
        + " ".join(keywords_list)
        + ". Common:"
    )
...
def summarise_neuron(
    index, neuron_layer, model_name, SIMILARITY_THRESHOLD, NUM_SYNONYMS
):
    ...
    if (
        cur_similarity < SIMILARITY_THRESHOLD
        and cur_phrase_similarity < SIMILARITY_THRESHOLD
    ):
        return
    ...
    try:
        new_phrases, new_avg_act = substitute_similar_phrases(
            max_phrases, acts, index, neuron_layer, NUM_SYNONYMS
        )
    finally:
