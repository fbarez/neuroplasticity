    ...
    for i in range(len(max_phrases)):
        ...
        try:
            for j in range(i, len(max_phrases)):
                ...
                similarities.append(model.wv.n_similarity(
                    phrase.split(), phrase_b.split()))
        except:
            continue
    if len(similarities) == 0:
        return 0
    ...
    for i in range(len(activation_phrases)):
        ...
        for word in phrase:
            most_similar = model.wv.most_similar(
                word, topn=NUM_SYNONYMS)  # Returns (key, similarity)
            similar_tokens = [ls[0]
                              for ls in most_similar]  # Extract similarity only
            for sub in similar_tokens:
